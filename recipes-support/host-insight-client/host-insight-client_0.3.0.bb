...
SRC_URI = "git://git@gitlab.com/hostmobility/ada/client.git;protocol=ssh;branch=main;name=client \
	git://git@gitlab.com/hostmobility/ada/proto.git;protocol=ssh;branch=main;destsuffix=git/proto;name=proto"
...
SYSTEMD_PACKAGES = "${PN}"
SYSTEMD_SERVICE:${PN} = "ada-client.service"
...
do_install() {
    install -d ${D}/opt/ada-client
    install -d 755 ${D}/${sysconfdir}/opt/ada-client
    install -d ${D}${systemd_unitdir}/system

    install -m 755 ${B}/target/${TARGET_SYS}/release/ada-client ${D}/opt/ada-client/ada-client
    install -m 644 ${S}/scripts/exit-handler.sh ${D}/opt/ada-client/exit-handler.sh
    install -m 644 ${S}/scripts/conf-fallback.toml ${D}/${sysconfdir}/opt/ada-client/conf-fallback.toml
    install -m 644 ${S}/scripts/ada-client.service ${D}/${systemd_unitdir}/system/ada-client.service
}

FILES:${PN} = "/opt/ada-client/ada-client /opt/ada-client/exit-handler.sh ${sysconfdir}/opt/ada-client ${sysconfdir}/opt/ada-client/conf-fallback.toml ${systemd_system_unitdir}/ada-client.service ${sysconfdir}/opt/ada-client"
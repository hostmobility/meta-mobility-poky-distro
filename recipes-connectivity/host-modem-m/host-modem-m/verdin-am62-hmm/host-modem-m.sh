...
CUSTOM_CONF=/opt/host-modem-m/wvdial.conf
CUSTOM_SECTION="[Dialer Custom]"
CUSTOM_DIALER="Custom"
...
if [ $RET = 0 ]; then
  if [ -f "$CUSTOM_CONF" ]; then
    if grep -qF "$CUSTOM_SECTION" "$CUSTOM_CONF"; then
      wvdial -C "$CUSTOM_CONF" "$CUSTOM_DIALER"
    else
      wvdial -C "$CUSTOM_CONF"
    fi
  else
    wvdial
  fi
